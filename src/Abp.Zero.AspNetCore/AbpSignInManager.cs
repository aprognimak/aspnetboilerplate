...
namespace Abp.Zero.AspNetCore
{
           ...
           where TUser : AbpUser<TUser>
    {
        ...
        public async Task<Guid> GetVerifiedUserIdAsync()
        {
            ...
            var identUserId = IdentityExtensions.GetUserId(authenticateResult.Identity);
            if (!string.IsNullOrWhiteSpace(identUserId))
            {
                return Guid.Parse(identUserId);
            }
        ...
        {
            return await GetVerifiedUserIdAsync() != Guid.Empty;
        }
